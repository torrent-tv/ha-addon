...
- **Fix**: Also in 2.80.2: what to download ahead of a viewer is stated once for the whole file, from the same map that places the encoders. Each read used to declare its own rolling window, and fifteen readers on a store that holds sixteen pieces then fought over memory — half of all evictions took a piece a reader had declared, two thirds of reads came back from disk, which is the other half of the empty picture above. A read now states only the piece it is stopped on; a piece leaves memory by how soon a viewer will need it, not by how long ago it was touched.
- **New**: The machine's whole budget is used. The film is divided between however many encoders this host can hold: first so the viewer does not stop, then so the rest of the film finishes as soon as possible. A paused viewer is going nowhere, so their film yields to whoever is watching.
...
- **Fix**: Also in 2.78.0: an encoder is no longer stopped for working ahead of the viewer. It used to be put to sleep two minutes in front of them and woken a minute later — two numbers nobody had measured — and that was the other half of the fault above. A film that is being encoded is now encoded to its end; where the viewer stands decides only what is made first and, through what this machine can afford, how many encoders work at once.
- **Fix**: How much must be ready before playback starts is measured rather than assumed. An encoder slower than the film loses time on every second played, so the shortfall over the rest of the film is worked out from its own measured speed and made in advance — at a quarter of realtime that is five minutes of film before a viewer sets off, where the old fixed two minutes would have run out partway through.
...
- **Fix**: One reading of an MP4 could loop for ever on a file whose last box declares its size as "to the end" — legal, and written by some muxers. Two copies of that reading existed, one guarded against it and one not; the guarded one is what remains.
